...
import { infoDescription, infoList } from '@/constants'
    ...
    <section className="relative z-10 mt-20" id="about">
      ...
      <div className="my-14 flex w-full flex-col lg:flex-row items-center gap-10 lg:gap-16">
        ...
        <div className="flex-1">
          ...
          <ul className="grid min-[378px]:grid-cols-2 lg:grid-cols-5 gap-4 max-w-4xl">
            {infoList.map(({ description, iconBlack, icon, key, title }) => {
              return (
                <li
                  key={key}
                  className="border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:-translate-y-2 duration-300 hover:bg-purple/80 hover:shadow-slate-700 hover:shadow-xl "
                >
